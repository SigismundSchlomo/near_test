...
use near_sdk::{AccountId, assert_one_yocto, env, log, Promise, require, near_bindgen};
...
impl StorageManagement for Contract {
    ...
    fn storage_deposit(
        ...
        if amount < min_balance && !already_registered {
            env::panic_str("ERR_DEPOSIT_LESS_THEN_MIN_STORAGE")
        }
    ...
    fn storage_unregister(&mut self, force: Option<bool>) -> bool {
        ...
        if let Some(account_deposit) = self.internal_get_account(&account_id) {
            require!(account_deposit.tokens.is_empty(), "ERR_STORAGE_UNREGISTER_TOKENS_NOT_EMPTY");
            self.accounts.remove(&account_id);
            Promise::new(env::predecessor_account_id()).transfer(account_deposit.near_amount);
            true
        } else {
            false
        }
