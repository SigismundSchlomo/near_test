...
use crate::{AccountId, Balance, Contract, StorageKey, TWENTY_TGAS};
...
impl Account {
    ...
    ///if given token is not registered and is not enough storage, deposit fails
    pub fn deposit_with_storage_check(&mut self, token: &AccountId, amount: Balance) -> bool {
    ...
    /// Withdraw amount of `token` from the balance
    /// Panics if the `amount` is bigger then the current balance
    pub fn withdraw(&mut self, token: &AccountId, amount: Balance) {
        if let Some(balance) = self.tokens.get(token) {
            require!(balance >= amount, "NOT_ENOUGH_TOKENS");
            let new_balance = balance - amount;
            self.tokens.insert(token, &new_balance);
        } else {
    ...
    pub fn unregister(&mut self, token_id: &AccountId) {
        ...
        assert_eq!(amount, 0, "{}", "NONE_ZERO_BALANCE");
    }
}
...
impl Contract {
    ...
    pub fn withdraw(
        ...
        assert!(amount > 0, "{}", "WITHDRAW_AMOUNT_MORE_THEN_BALANCE");
        let sender_id = env::predecessor_account_id();
        let mut account = self.internal_unwrap_account(&sender_id);
